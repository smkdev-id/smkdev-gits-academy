...
<header
    class={"sticky z-[10] top-0 duration-200 lg:px-24 px-6 flex items-center justify-between border-b border-solid " +
        (y > 0
...
>
    ...
    <div class="desktop-menu items-center gap-4">
        ...
        <a href={tab.link} class="blueShadow relative overflow-hidden px-5 py-2 group rounded-full bg-indigo-700 text-slate-950">
            <div
...
<nav class="mobile-nav bg-white shadow-lg rounded-md mt-2 px-6 py-4">
    {#each tabs as tab, index}
        <div class="relative mb-2">
            {#if tab.name == "Learn"}
                <p class="block px-4 py-2 text-indigo-700 hover:bg-violet-100" on:click={toggleMobileLearnDropdown}>
                    {tab.name}
                </p>
